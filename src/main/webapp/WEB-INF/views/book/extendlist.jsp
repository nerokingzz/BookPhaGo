	...
	<c:choose>
		<c:when test="${booklistSize gt 0 }">
			<c:forEach var="i" begin="1" end="${booklistSize }">
				...
				<input type="text" style="border:none;border-right:0px; border-top:0px; boder-left:0px; boder-bottom:0px; width:100%;" size="20" name="bookNumber" value="${booklist.get(i-1).get('BOOKNAME') }" readonly>를 연장하시겠습니까?
			</c:forEach>
		</c:when>
